...
async function saveUserResult(username, scoreResult) {
  const {
    totalScore = 0,
    tweetCount = 0,
    countedTweets = 0,
  ...
  // Ranked sorted set (score-only, for fast top-N reads).
  await redis.zadd("leaderboard", { score: totalScore, member: username });

  // Full detail hash. @upstash/redis auto-serializes objects/arrays on write
  // and auto-deserializes them on read.
