...
// Raw, unweighted human engagement on a single tweet (no views/bookmarks).
// Used to filter out accounts that scored via follower authority but got no
// actual reaction from anyone.
function rawEngagementOf(t) {
  return (
    (t.likeCount ?? 0) +
    (t.retweetCount ?? 0) +
    (t.quoteCount ?? 0) +
...
function scoreUserMindshare(tweets) {
  ...
  // Highest raw human engagement across the counted (top-10) tweets.
  const maxRawEngagement = top10.reduce(
    (max, t) => Math.max(max, rawEngagementOf(t)),
    0
  );
  ...
  const topTweets = top10.slice(0, 3).map((t) => ({
    ...
    rawEngagement: rawEngagementOf(t),
  }));
